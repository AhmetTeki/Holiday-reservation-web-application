...
namespace TraversalRezervasyonApp.Areas.Admin.Controllers
{
    ...
    public class VisitorApiController : Controller
    {
        ...
        public async Task<IActionResult>  AddVisitor(VisitorViewModel model)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5126/api/Visitor",content);
        ...
        {
            var client= _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5126/api/Visitor/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client= _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5126/api/Visitor/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData=await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateVisitor(VisitorViewModel model)
        {
            ...
            var jsonData=JsonConvert.SerializeObject(model);
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:5126/api/Visitor", content);
